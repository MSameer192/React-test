...
import { addNewCustomer, updateNewCustomer } from "../customerSlice";
import InputTextUpdated from "../../../components/Input/InputTextUpdated";
import { toast } from "react-toastify";
import axios from "axios";
import { CLOUD_NAME, CLOUD_PRESET } from "../../../app/constants/constants";
...
function CustomerModalBody({ closeModal, extraObject }) {
  ...
  const submitCustomer = (submit) => {
    if (submit === "Update") {
      dispatch(
        updateNewCustomer({
          id: extraObject.id,
          updatedData: { ...customerObj, avatar: imageUrl },
        })
      );
      ...
      } else if (customerObj.last_name.trim() === "") {
        return setErrorMessage("Last Name is required!");
      } else if (customerObj.email.trim() === "") {
        return setErrorMessage("Email is required!");
      } else {
        // Generate a unique ID for the new customer (you may use a library like `uuid`)
  ...
  function generateUniqueId() {
    ...
    // Get the last item's ID from customersData in local storage
    const lastItemId = customersData[customersData.length - 1]?.id || 0;

    // Increment the last item's ID to generate a new unique ID
  ...
  return (
    <>
      ...
      <div className="modal-action">
        <button
          className="p-3 text-white rounded-xl font-medium text-lg border-0 bg-gradient-to-r from-[#57BC90] to-[#015249] w-full"
          onClick={() =>
